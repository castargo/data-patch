...
* ```find <dir>``` - вывести список всего содержимого директории и ее дочерних директорий рекурсивно
  ...
  * ```find ~/ -atime +100 -exec rm -i {} \``` - можно выполнять действия с нейденными файлами
...
Ядро хранит информацию обо всех процессах, чтобы упорядочить их работу. Каждый процесс имеет ID - PID.
...
* [Интерактивные просмотрщики процессов](https://wiki.dieg.info/top):
  * top - интерактивный просмотрщик процессов
    ...
    * По умолчанию она выдает задачи, наиболее загружающие процессор сервера
    * Обновляет список каждые две секунды
  * htop - аналог top
